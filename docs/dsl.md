...
_Design record — where each sugar is lowered and tested. Skip this unless you're
changing how the DSL works._

**Status:** implemented (2026-07-05). Every feature lowers before
`ApplyDefaults → Compile → Validate → CheckContracts → DryRun`, so the existing
pipeline enforces the result; `steps validate --print` shows the expansion. Two
...
trace parity (transition counts, state sequences, and — for `evidence:` — the
composed prompts observed at the model boundary).
