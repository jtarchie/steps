...
- **Pinned across runs, reconsidered per run, and never forever.** Once a source (preflight-picked or mid-run-picked) has *served* one run, a `steps watch` process keeps using it rather than re-failing-over on every poll. A pin belongs to one `(pipeline, agent)` pair: a process serving several pipelines that each declare a `reviewer` keeps their outages apart.
  ...
  **Only a fact established *just now* ends a pin.** The probe that says the primary recovered may itself be a cache hit up to `defaults.preflight.cache:` old (5m by default), and a positive from *before* an outage would otherwise release a pin the cascade earned *during* it — sending the step back to the still-broken primary every poll until the cache entry aged out. Keeping a pin on a stale reading costs nothing, so a stale reading may keep one; it may never release one. The same rule applies to the pinned source's own probe, in the other direction.

  *Just now* means this pre-run probe, not this request. Several agents legitimately share one endpoint and model, and the probe of it is collapsed to a single request — so the first agent sends it and the rest read the same answer back. All of them hold a fact from this probe; counting only the sender would leave every other agent on the same model unable to ever return to it.
