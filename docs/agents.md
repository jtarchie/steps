...
4. Exit when the model stops requesting tools, `max_turns` is exceeded, or [loop detection](agents-internals.md#loop-detection) kills a stuck conversation.
   - **Before any limit runs out, the model is nudged once to wrap up, with its tools still granted.** Whichever of the step's limits runs low first sends it: a fifth of the `timeout:` left or less than two of the slowest model request this attempt has made, whichever is more (a reasoning model can think longer on one request than the whole fifth), a fifth of `max_turns` left (never fewer than two turns, one to write what it owes and one to say so), or the token `budget:` close (below). One nudge per attempt, whatever the cause. It exists because the wrap-up request below has no tools: a model that learns of the cap only then cannot write the file it was asked for — measured on pr-review, where six reviewers ran into `max_turns` unwarned and five left no report.
   - A spent turn budget **ends** the conversation rather than destroying it: the runner makes one final request with the tools withheld, asking the model to answer from what it already gathered, and records the answer with `wrapped_up: true` so a degraded answer is tellable from a confident one. The run page marks the step **stopped early**. That answer is better than nothing, but it is not a deliverable: a step with `assert: {files: [...]}` still fails if the file was never written, which is what makes the salvage safe to count as a success when it is one.
...
- **`args:` pinning**: a pinned key is excluded entirely from the schema the model sees. At execution, the pinned value is merged in *over* whatever the model supplied, before rendering and before the missing-argument check. The model chooses *when*, the pipeline chooses *where*. `args:` is also valid on a single-tool MCP grant, where each key must be a property the server declares — see [mcp.md](mcp.md#pinning-arguments-args).
...
**A hosted model is warned before its own ceiling, once** (the shared wrap-up nudge, above). When a fifth of the agent's allowance is left, or less than two more turns the size of the last one, the next request carries a message saying how much is spent and asking it to answer from what it has. Tools stay granted, like the `timeout:` warning. There is no tools-withheld wrap-up after a breach the way there is for a spent `max_turns:`: one more request would spend past the cap the budget exists to hold. The job's ceiling does not warn, and a CLI agent's `usd` is enforced inside its own process, out of reach.

**A final answer that crosses the ceiling is kept.** Every request re-sends the whole conversation, so the closing response costs a full turn — and it is the last thing the step spends on. Failing it saves nothing and deletes the answer (measured: a support reply written to disk, then the step red for the "done" that followed). So a breaching response that asks for **no tools** is accepted and the overshoot logged as `agent.budget_overshot`; one that asks for tools would spend more and act on the world, and stops the step as before. This is the agent's **own** ceiling only: a final answer that crosses the **job's** still fails the step, because the job would go on to spend on the steps after it. A compaction summary that crosses either ceiling stops the step too: it always has another request behind it, so there is nothing to keep it for ([agents-internals.md](agents-internals.md#compacting-long-conversations)). The overshoot is bounded by one response either way — which is what it always was, since the request that crosses has already been paid for by the time the ceiling is checked. A budget is a backstop against a runaway, not a number the model can plan against: it cannot see its own context size. What shapes a conversation's spend is `max_turns:` times `max_context_bytes:`, and those are the dials to set.
...
- **Pinned across runs, reconsidered per run, and never forever.** Once a source (preflight-picked or mid-run-picked) has *served* one run, a `steps web` process keeps using it rather than re-failing-over on every poll. A pin belongs to one `(pipeline, agent)` pair: a daemon holding several pipelines that each declare a `reviewer` keeps their outages apart. The pipeline half is the same name the state database, the `/p/<slug>` route and every run record use — one identity, chosen by whoever ran `steps pipeline set -p <name>` (see [web.md](web.md)) — so a pin log line's `pipeline=` joins to the run you are correlating it against.
  ...
  **A pin is re-decided at the pre-run probe.** For a pinned agent, preflight probes *both* the primary and the source actually in use, and either fact ends the pin: the primary answering again (`agent.failover.returned` — the outage is over), or the pinned source no longer answering (`agent.failover.pin_lost` — re-decide from the primary rather than let the next step discover it by failing). While the primary is still down and the fallback is still serving, nothing changes — including the pin's *position* in the list, so an agent that cascaded past two unwell entries is not quietly walked back to the first one that answers a probe. The mid-run cascade remains the other place a pin moves, under the stricter rule above.

  All three edges are as loud as `agent.failover`'s own log line, on purpose: an operator watching for a change in model quality needs to see the return as well as the departure, or a quality shift gets attributed to the wrong change. They are log-only, though — a release happens before any step exists to carry it on an output line or into a recorded result.

  **Only a fact established *just now* ends a pin.** The probe that says the primary recovered may itself be a cache hit up to `defaults.preflight.cache:` old (5m by default), and a positive from *before* an outage would otherwise release a pin the cascade earned *during* it — sending the step back to the still-broken primary every poll until the cache entry aged out. A stale reading may leave a pin standing; it may never release one. Leaving one standing is not the same as vouching for it, though — an agent whose primary is freshly dead and whose pinned fallback was last seen dead is reported as a problem rather than passed over, so a job does not start green on two sources preflight believes are down. The same rule applies to the pinned source's own probe, in the other direction.
  ...
  **A pin also expires on its own, after 15 minutes.** This is the exit that does not depend on preflight running at all: an agent with `preflight: false`, a pipeline with `defaults.preflight.disabled:`, and a run under `--no-preflight` all skip the pre-run probe, while the mid-run cascade that *installs* a pin is gated by none of them — so without this, a ninety-second blip pinned a long-lived `steps web` for the life of the process, in exactly the configuration this page recommends for a cold local model. Expiring re-decides from the primary with no probe behind it (`agent.failover.pin_expired`), which is the honest cost of having no probe: one `attempts:` cycle against a source that may still be down, at most once per 15 minutes per agent.
  ...
  Deciding at the preflight boundary is what keeps a *flapping* primary from splitting one job across two models: a run decides before it starts, and one job at a time in a process sees a settled source throughout. (`steps web --max-concurrent` above 1, and a `steps web` serving several pipelines, run jobs concurrently, so a second job's pre-run probe can still move a first job's agent between steps — within that pipeline.) It costs no new tunable and one extra probe per run per pinned agent, only while the primary is still down: `defaults.preflight.cache:` bounds how often that becomes a real request.
