...
- **Pinned for the process, reconsidered per run.** Once a source (preflight-picked or mid-run-picked) has *served* one run, a `steps watch` process keeps using it rather than re-failing-over on every poll.
  ...
  Both directions take positive evidence. A source is pinned only after it carried a conversation to an end, which a provider answering `400` did not do. Mid-run, a pin is dropped only after the cascade actually *tried* the alternatives and none of them served; a step that failed without ever swapping has learned nothing, and dropping the pin there would send the next step back to a primary the probe may already have found dead. Anything ambiguous — the step spending its own `timeout:`, the run being cancelled — changes nothing in either direction.

  **A pin is re-decided at the pre-run probe, and nowhere else.** For a pinned agent, preflight probes *both* the primary and the source actually in use, and either fact ends the pin: the primary answering again (`agent.failover.returned` — the outage is over), or the pinned source no longer answering (`agent.failover.pin_lost` — re-decide from the primary rather than let the next step discover it by failing). While the primary is still down and the fallback is still serving, nothing changes.

  Both edges are as loud as `agent.failover` was, on purpose: an operator watching for a change in model quality needs to see the return as well as the departure, or a quality shift gets attributed to the wrong change.

  Restricting this to the preflight boundary is what keeps a *flapping* primary from splitting one job across two models — a run decides once, before it starts, and a step's source is settled when it starts. It costs no new tunable and at most one extra probe per run per pinned agent: preflight was already asking the primary this question every run and discarding the answer, and `defaults.preflight.cache:` still bounds how often that becomes a real request.
