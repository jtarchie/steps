...
- **Pinned for the process, reconsidered per run.** Once a source (preflight-picked or mid-run-picked) has *served* one run, a `steps watch` process keeps using it rather than re-failing-over on every poll.
  ...
  **A pin is re-decided at the pre-run probe.** For a pinned agent, preflight probes *both* the primary and the source actually in use, and either fact ends the pin: the primary answering again (`agent.failover.returned` — the outage is over), or the pinned source no longer answering (`agent.failover.pin_lost` — re-decide from the primary rather than let the next step discover it by failing). While the primary is still down and the fallback is still serving, nothing changes — including the pin's *position* in the list, so an agent that cascaded past two unwell entries is not quietly walked back to the first one that answers a probe. The mid-run cascade remains the other place a pin moves, under the stricter rule above.
  ...
  Both edges are as loud as `agent.failover`'s own log line, on purpose: an operator watching for a change in model quality needs to see the return as well as the departure, or a quality shift gets attributed to the wrong change. They are log-only, though — a release happens before any step exists to carry it on an output line or into a recorded result.

  Deciding at the preflight boundary is what keeps a *flapping* primary from splitting one job across two models: a run decides before it starts, and one job at a time in a process sees a settled source throughout. (`steps watch --max-concurrent` above 1, and a `steps web` serving several pipelines, run jobs concurrently against one process-wide pin, so a second job's pre-run probe can still move a first job's agent between steps.) It costs no new tunable and one extra probe per run per pinned agent, only while the primary is still down: `defaults.preflight.cache:` bounds how often that becomes a real request.

  Two gaps worth knowing. An agent with `preflight: false`, a pipeline with `defaults.preflight.disabled:`, and a run under `--no-preflight` all skip the pre-run probe — so a pin the *mid-run* cascade installs in one of those has no exit condition and lasts for the life of the process, as every pin used to. And the probe answer that releases a pin can be up to `defaults.preflight.cache:` old, so a primary that died inside that window reads as recovered until the entry expires.
