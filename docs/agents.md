...
- **Pinned across runs, reconsidered per run, and never forever.** Once a source (preflight-picked or mid-run-picked) has *served* one run, a `steps web` process keeps using it rather than re-failing-over on every poll. A pin belongs to one `(pipeline, agent)` pair: a process serving several pipelines that each declare a `reviewer` keeps their outages apart.
