...
|---|---|---|
| [resources.md](resources.md) | Resources and resource types: the built-ins `git` and `cron`, the `check`/`in`/`out` contract, `version:`, `trigger:` | short |
| [webhooks.md](webhooks.md) | `type: webhook`: deliveries as versions, the provider table, `filter:`/`id:`/`version:`, custom schemes, tunnels, `--deliver` | medium |
...
steps run <pipeline>        run one job (--resume <id> continues a failed one,
                            --rerun <id> runs it again against its own versions,
