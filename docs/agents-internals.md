...
**Timeout** bounds the **entire conversation** (all turns, all tool calls, and any request retries within them) to a wall-clock deadline. The built-in `agentStepTimeout` is 30 minutes; `timeout:` (if set) overrides it. A timeout mid-conversation classifies as **failed**, per Concourse — the step spent its budget without concluding (see [attempts-timeout.md](attempts-timeout.md#timeout-classification)).
...
`prepareAgentStep` is otherwise untouched: workspace, `buildAgentTools`, `injectSynthesizedTools`, context blocks, and the spill directory are all built identically. Only `newAgentLLM` is skipped — a CLI source has no HTTP client, so the OpenRouter/repair/retry transport stack has nothing to wrap. `config.cliProviders` holds what a *load* needs: which `@name` values exist and which binary to look for. There is no second, invocation-side table any more (see "The bridge" below) — the CLI's own native tool mapping used to live in `agent.cliRuntimes`, and deleting the mapping deleted the table with it.

### The bridge

The CLI is passed `--tools ""` **unconditionally**, and `clibridge.go` starts an MCP server in the parent process — on a loopback listener with an ephemeral port, for the duration of one attempt — that re-exports **every** tool in the step's registry, built-in or custom. There is no longer a "the CLI serves this one natively, so skip it" branch: the bridge is the entire tool surface, not a side channel next to a smaller native one. It writes a `--mcp-config` document naming its URL. The config file goes to the OS temp dir, never the workspace — the workspace is captured as artifacts and readable by the agent's own file tools, and a live callback URL belongs in neither.

Always loopback, unconditionally: the CLI process is always a host subprocess of this one, whether or not the step names an `image:` for its tools (see [infra.md](infra.md#cli-agents)), so there is no longer a containerized-CLI case that needs a wider bind to be reachable from. A bridged tool's own container, if `image:` names one, is where the *tool* runs — the same `toolEnv.runner` a hosted agent's tools already run through — never where the CLI does.
...
The child's tool surface used to be two questions — `--tools` (which built-ins exist at all) and `--allowedTools` (which of those need no further prompt) — and conflating them was the mistake worth avoiding. There is one question now: `--tools ""` is passed **unconditionally**, so the CLI offers no built-in of its own regardless of the grant (verified — `--tools ""` reports an empty session tool list), and `--allowedTools` names every granted tool under the bridge's `mcp__steps__*` namespace, since there is no shorter native spelling left to prefer. Deny-by-default falls out of the empty value itself: a built-in this build has never heard of is withheld because `--tools` never grows a case for it, not because a per-grant computation remembered to exclude it.

### Attestation: the init event is the fence

`--tools ""` is policy inside an upstream binary this build does not pin, whose built-in surface grows with releases — so the fence is enforced, not merely asked for. The CLI's stream-json `init` event (`type: "system", subtype: "init"`) lists the session's tools, and `cliattest.go`'s `checkCLIToolSurface` compares it against exactly the bridged grant (`cliToolPermissions`) — by **set equality**, not a subset check, so a tool the grant expected but the child never reported is caught too, and "the bridge never connected" (an empty report against a non-empty grant) is subsumed for free.

The check runs the instant `parseCLIStream` parses the `init` line, not after the stream ends, and a mismatch kills the child immediately (`execCLI` cancels its own derived context) before any later line — any further tool call — is even read. The absence rule has a precedence fix built in: a `result` event with no preceding `init` **is** an attestation failure (the child finished without ever proving its tool surface), but a stream that produced neither event stays the ordinary "exited without reporting a result" case — a child that died before saying anything was never in a position to misreport its tools either.
...
Tool names enter the trajectory **de-namespaced**: `mcp__steps__read_file` records as `read_file`, identical to what a hosted step's own trajectory shows for the same call. This reverses the old rule ("names are recorded exactly as the CLI reported them") deliberately: `mcp__steps__` is our **own** bridge's namespace (see `cliBridgeServerName`), so stripping it is de-namespacing an observation, not translating one into something it wasn't — the thing that ran really was steps' `read_file`. A name with no such prefix (`Bash`, `Task`, anything a surplus native reports despite `--tools ""`) is kept **verbatim** rather than guessed at, doubling as a second, human-readable signal of an attestation failure (see above). This is what lets one `assert.tool_calls: [{name: read_file}]` pass on a hosted step and a CLI step alike — the issue #100 goal.
...
- **`node_transcripts`** carries the full exchange, keyed by the same node hash: the system prompt, the opening task message and every later `messages:` entry, the upstream/context_paths synthetic tool exchanges built into the opening request, the model's visible text each turn (including commentary accompanying tool calls, which the trajectory drops), every tool call, every tool result (truncated to 4,096 bytes apiece — the model may have seen up to 32,000), and sub-agent delegations with the child conversation's own events nested inside. It is read on demand ("what did this step actually say and do"), never on the run path.
...
One path never writes one: `RunHook`/`RunFix` (the no-record contract — the enclosing step records the aggregate outcome). CLI-backed conversations (`cli.go`) write one too: `parseCLIStream` records the child's own text/call/result events off its stream (clistream.go), and `cli.go` records the system prompt and each message it sends the child directly, gated so a retry, a nudge round, or a resumed session never re-records what a prior invocation already sent.
