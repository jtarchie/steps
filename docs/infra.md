...
- **A preempted GCE worker drains exactly as a spot EC2 one does.** The shim watches the instance metadata service — both clouds answer the same link-local address, and the shim tells them apart once per session — and GCE's `preempted` flag flipping to TRUE is relayed as a terminal draining notice. The warning is shorter (about thirty seconds against EC2's two minutes) and there is no rebalance-recommendation analog, so every GCE notice is terminal.
