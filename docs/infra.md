...
- **A worker whose filesystem has no executable bit strips it, silently — so Windows workers are not supported.** A tree crosses the wire carrying each file mode, but Windows has nothing to store `0111` in: `os.Chmod` there sets only the read-only attribute and returns no error, and `os.Stat` reports every regular file as `0666` (or `0444`). So the tree unpacks without its executable bits, the repack on the way home reads that back off the filesystem, and what returns is a tree whose scripts are no longer scripts. The step cache sees content that changed for no reason a reader can point at, and a later step that tries to run the file cannot. Nothing raises an error at any stage. This is a property of the *worker's filesystem*, not of the CPU: a Linux or macOS worker of any architecture is unaffected.
