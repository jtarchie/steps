...
- **The fetched tree comes home, and stays on the worker.** A placed `in:` fills a directory on the worker and the whole of it is brought back, so a later step — here or on another worker — reads it exactly as it would a local fetch, and the resource cache digests it identically. The worker also files what it just packed under the digest the next step's offer will name, as it does for everything it receives, so a later step placed on the **same** worker is answered "already here" and the tree crosses the wire once rather than twice. The same holds for a placed task's declared outputs. A worker whose scratch is on tmpfs keeps nothing it produced — that cache would be memory. The cost that remains is that bytes still travel worker → orchestrator → a *different* worker; there is no worker-to-worker streaming.
