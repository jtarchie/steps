...
- **A `root:` whose filesystem cannot hold an executable bit is refused**, and measured rather than assumed: startup writes a file under the root, sets `0700`, reads the mode back, and removes it. The bit is content — the step cache hashes it — so a root on vfat or exfat (whose `fmask`/`dmask` synthesize a fixed mode), on CIFS without unix extensions, or on 9p/virtiofs under Lima or Docker Desktop would hand back trees whose scripts are no longer scripts, with the cache reading the loss as an edit and nothing reporting it. This is the same refusal a worker gets at the handshake (see [infra.md](infra.md)), for the same reason, on a directory you name rather than a machine you dial.
...
- **A resume rebuilds each build against every version it was created with.** Every get's version is recorded per build as the build starts — fixed gets and `--pin` included, as Concourse records every input of a build — and a resume binds each build to exactly those, never to what the check reports now: a branch re-pushed while the run sat failed is still built at the commit the run began with, and a pinned run resumes with its pin. What no longer exists cannot be rebuilt, so a build whose version `version_history:` has since pruned refuses the whole resume, naming the build and the version, rather than choose another; start a new run for the versions that remain. A run that failed before its first get created any build has nothing recorded and resolves as a new run would.
