...
Every step runs isolated: its working directory is materialized from its own declared `inputs:` and nothing else, and only its declared `outputs:` are captured back. A `get` fetches into the build's artifact store; a `task`/`agent`/`put` step sees an artifact only by naming it. There is no shared mutable directory and no way to opt into one — an artifact reaching a step that never declared it would be ambient data flow, and every flow in this DSL is opt-in. The `workspace:` block is optional tuning for *how* trees are materialized, not a switch that turns isolation on:

```yaml
workspace:
  strategy: copy   # the default; or: btrfs (Linux only)
  root: /path       # optional for copy (default: system temp); required for btrfs
  options:
    compression: zstd   # btrfs only: zstd | lzo | zlib | none
```

- **`inputs:`/`outputs:`** on a `task`/`agent`/`put` step (and on a top-level `tasks:` entry, overridable per step the same way `fix:` is) name artifacts — a resource an earlier `get` fetched, or an output an earlier `task`/`agent` produced. A step's working directory contains an `<input>/` copy (or, on btrfs, an instant copy-on-write snapshot) of each named input plus an empty `<output>/` dir per declared output, captured back into the build's artifact store after the step succeeds. `put` steps compose a read view the same way from their own `inputs:` — there's no implicit "all artifacts so far" view (but see `inputs: all` below).
- **`inputs:`/`outputs:` are optional and default to empty.** An absent `inputs:` means the step mounts nothing; there is no requirement to write `inputs: []`. A pure-compute step legitimately declares nothing. An agent step's `dir:` also names the artifact it works in (its first path component) and is validated the same way.
- **Declared inputs are validated against producers.** `ValidateArtifactFlow` runs for every job (even under `--force`): an `inputs:` (or agent `dir:`) naming an artifact nothing earlier fetched/produced is an error. This turns "this step reads an artifact nobody produced" — e.g. an agent told to summarize a repo that was never fetched — into a plan-time failure, before any command or model runs.
- **A name in both `inputs:` and `outputs:` is read-modify-write.** The directory is materialized from the artifact's current content and captured back over it after the step succeeds. This is how a revise loop (`to: failure: draft`) carries state between visits: the looping step declares the same artifact as input and output and each visit continues from the last captured state. A step that *fails* captures nothing, so put the state-advancing write in a step that succeeds (see `examples/flow.yml`'s `converge` job).
- **Name mapping (`input_mapping:`/`output_mapping:`, task steps only).** `{task-config-name: plan-artifact-name}`, mirroring Concourse: a reusable `tasks:` entry with pinned input/output names can be pointed at whatever a job actually fetched/produced without editing the task. The directory on disk keeps the task-config name; the artifact copied in / captured out uses the plan name. Mapping keys must be a subset of the resolved task's declared inputs/outputs, and mapping values must be plain artifact names.
- **`outputs:` on an `across:` step collects**: each cell's capture lands under the cell's own coordinates inside the declared artifact (`findings/alpha/…`), so a matrix's clone-cells share one output name without clobbering each other — see [control-flow.md](control-flow.md#collected-outputs-outputs-on-the-matrix). Requires `strategy: copy` (the default).
...
- **A resumed run continues in the failed run's build tree.** The provider reuses the kept directory — its artifact store holds what the finished steps captured, and per-step directories are rebuilt per step anyway, which is what makes an isolating workspace resumable.
