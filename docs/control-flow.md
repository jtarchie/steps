...
A step routes to another step **in the same get-segment** based on its outcome, including jumping backward to form a bounded loop (a judge/revise cycle). Two spellings, one per kind of outcome: a `task`/`put`/verdict-less `agent` carries `to:`, a binary `success`/`failure` map; an `agent` carries `verdicts:`, an ordered list that declares its vocabulary **and** its targets together. Task loops are self-verifying via `examples/flow.yml`; verdict routing is illustrated in `examples/agents.yml` (needs a model).

```yaml
- agent: critic
  verdicts:
    - approve: publisher    # route: record the verdict and jump
    - revise:  writer       # backward — the loop max_visits: bounds
    - escalate              # bare: record it and carry on in declaration order
    - failure: escalator    # reserved: the step errored or decided nothing
  max_visits: 3
```
...
- **Agent verdicts (`verdicts:`)** is the N-way judge. A step declaring verdicts gets a synthesized *required* `verdict` tool whose enum is exactly the declared names in declaration order — the model must call it, reusing the required-tool `tool_choice` forcing from [agents.md](agents.md) verbatim. The choice becomes the routing key. Agent-only (an `ensemble:` counts — its members vote and the block routes on the decision).
  ...
  - **`to:` and `verdicts:` are mutually exclusive.** A step carrying both is the pre-cutover spelling and is refused at load, with the new form in the message (`examples/invalid/verdicts-with-to.yml`).
- **`next`** is the one target that is not a step name: *continue in declaration order*. It exists because a verdict must name a target and a **container has none** — `in_parallel:`, `race:`, `across:`, `ensemble:` and `approval:` have no name to be jumped to, so without it an author whose next step is one of those has to route *past* it. That is not equivalent, and quietly so: routing past an `approval:` skips the human gate the pipeline exists to have, while reading like it merely moved on. `next` is always forward, so it never requires `max_visits:`, and it is a real route rather than a fall-through — `failure: next` **consumes** the failure, which is how a step says "carry on anyway". The word is reserved on the value side: a step actually named `next` inside a `to:`-using segment is a load error naming the collision. `examples/flow.yml`'s `fall-through` job is the runnable version.
...
- **`assert.verdict`**, on an agent step that declares `verdicts:`: the name the model must have emitted, matched exactly. A classifier's product *is* the choice, and `tool_calls` could only assert the verdict tool was called, not what it chose — so a fixture used to pass on whatever the model decided. Naming a verdict outside the declared list, or setting it on a step with no `verdicts:`, is a load error: the assert could never match on any run (`examples/invalid/assert-verdict-not-declared.yml`).
...
- There's no modelless agent fixture for `assert.tool_calls` in `examples/` — a `steps test` fixture can't point an agent at a stub, since `source.endpoint:` is a credential boundary and isn't templatable. It's covered instead by unit tests plus the end-to-end tests in `e2e_test.go`, which drive a scripted OpenAI-compatible endpoint (`fakeprovider_test.go`) through a real `run()` and assert on the trajectory, the verdict route, and the recorded outcome. `on_error` and `on_abort` now have deterministic triggers, and they took different forms. `on_error` is a YAML fixture — `examples/flow.yml`'s `timeout-fires-on-error`, where a per-attempt `timeout:` cancels the ATTEMPT's context while the job's stays live, which `outcome.Classify` reads as errored rather than failed (no docker or bad image needed, which is what had blocked it). `on_abort` cannot be a fixture at all: it needs a cancelled JOB context, and a pipeline file has no way to ask for one — so it is `TestConformanceAbortFiresOnAbortHook` in `internal/pipeline`, which cancels mid-step. Both assert the *other* hook did not fire, since the risk being guarded is a classifier that treats everything non-green alike.
