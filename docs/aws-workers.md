...
[`hack/aws-fixture.sh`](../hack/aws-fixture.sh) does all of this in one command for this repo's own tests. This page is the same thing typed out, so you can see what each resource is for and adapt it. Every command is `aws` CLI v2 with credentials already configured.
...
The whole machine shape in one object. The user data installs docker at first boot — **only needed if you want `image:` on a placed step**, which runs the command in a container on the worker. Omit it and everything else still works.
...
```bash
ID=$(aws ec2 run-instances --launch-template "LaunchTemplateName=$NAME" --count 1 \
  --query 'Instances[0].InstanceId' --output text)

aws ec2 wait instance-running --instance-ids "$ID"
```

Launches one machine from the template and waits for EC2 to call it running. **Running is not reachable** — the SSM agent still has to register, which takes another minute or two:
...
steps run \
  ...
  --worker "burst=aws://launch/$NAME?version=1&binary=/tmp/steps-linux-arm64&region=$AWS_REGION" \
...
- **`aws://launch/…?version=1`** — acquires a machine from that template version for the job and terminates it at the end. Acquisition is **per job, not per step**: the first placed step pays for the machine and the rest reuse it.
...
aws ec2 delete-launch-template --launch-template-name "$NAME"
