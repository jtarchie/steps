...
- **The tree hash covers tracked, modified and untracked-but-not-ignored files** (`git add -A` into a scratch index — the real index is never touched).
- **One run may be committed as a SERIES.** `pre-commit` accepts the whole validated tree, or any slice of it *provided the working tree is still exactly what the run saw*. The first version demanded the whole tree in one commit; with an eight-minute run that taught one lesson, which was to bundle unrelated changes. An edit made after the run — staged or not, in this commit or not — still refuses everything, because then nothing in the working tree is vouched for.
- **`pre-push` is where the series has to add up**: every tip being pushed must be EXACTLY the validated tree. That is where the series allowance moved the forgotten-`git add` refusal to — a file the tests ran with and nobody committed is "works on my machine", and it is caught on what actually leaves the machine. The intermediate commits of a series are NOT individually proven; only the tip is. A push of a tag or an older commit is refused for the same reason, which is the honest answer.
...
At commit: something changed after the run — rerun `task`. At push: a `D` line is a validated file nobody committed (commit it; no rerun needed, the working tree is still the validated one), anything else means rerun `task`. That is the whole procedure. A human can `git commit --no-verify` and owns that decision; the agent cannot, and should ask rather than look for a third way.
