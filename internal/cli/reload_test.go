...
// Hot-reloading the pipeline file, from the inside.
//
...
// ConfigWatcher directly — edit the file, run one check, and assert what the
// served pipeline now is and what the store kept. The two tests that drive a
// REAL daemon through `steps web` instead live in ./e2e, which is the only
// place that proves the watcher is wired to the command at all.
