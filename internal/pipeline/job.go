...
func RunJob(ctx context.Context, cfg *config.Config, job *config.Job, pinned map[string]string, provider workspace.Provider, st store.Store, skipCache bool) error {
	...
	// One set of venue leases per job: a worker that has to be acquired is
	// paid for by the first placed step and given back here, however the job
	// ends. Released under a context of its own, because a cancelled job is
	// exactly when a machine left running would be worst.
	...
	err = recordRunIdentity(ctx, st, resume, job.Name, workspaceRoot, cfg.Revision)
	if err != nil {
		return err
	}
...
func prepareImages(ctx context.Context, cfg *config.Config, jobName string) error {
	...
	// this machine's docker CLI aimed at the worker's daemon through the
	// forwarded socket, and a missing binary had to be found before a machine
	// was acquired and billed and the tree pushed. The container is created
	// over the socket directly, so a daemonless — and now docker-less —
	// orchestrator running placed steps is simply a working arrangement, and
	// there is nothing left to check ahead of time.
	if !cfg.UsesImages() {
		return nil
	}
...
func runJobPlan(
	...
	// build began with — rather than against whatever is newest, and it does
