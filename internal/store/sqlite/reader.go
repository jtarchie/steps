...
// Reader reads across every pipeline in one state file — see store.Reader for
// why crossing them is a separate type at all.
//
...
func OpenReader(path string) (*Reader, error) {
	...
	if found != schemaVersion {
		...
		if empty {
			return nil, fmt.Errorf("%w: %s", store.ErrNoState, path)
		}
...
func OpenExisting(path, pipelineName string) (*Store, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		...
		return nil, fmt.Errorf("%w: %s is not in %s, which holds: %s",
			store.ErrNoSuchPipeline, pipelineName, path, held)
	}
...
func (r *Reader) RecentRuns(ctx context.Context, pipelines []string, limit int) ([]store.CrossRunRow, error) {
	...
	return collect(ctx, r.db, "runs across pipelines", query, args, func(rows *sql.Rows) (store.CrossRunRow, error) {
