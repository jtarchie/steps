// Package sqlite is the sqlite driver for the state database: the schema, the
// SQL and the connection pragmas behind internal/store's contract.
//
// It owns those end to end rather than sharing a dialect with a second driver,
// which is what lets each use its database's own answers — sqlite's rowid for
// insertion order here, a serial id and SKIP LOCKED elsewhere — instead of the
...
// The assertion is here rather than at a call site so a method that drifts
// from store.Store is a build error in this package.
...
// Description is the state file behind this handle. Two handles reporting the
// same description share a file — which is what a shared state file makes
// possible, and what lets a caller ask one Reader about several pipelines at
// once instead of querying each and merging.
//
// A path is already safe to print, so this driver has nothing to redact; a
// network driver returns a credential-free URL for the same string's purpose.
func (s *Store) Description() string { return s.path }
...
func schemaVersionError(path string, found int) error {
	return fmt.Errorf("%w: %s is at schema %d and this steps writes %d — there is no upgrade path (see the schema's own comment); delete the file and run again, which loses that pipeline's run history and cache but nothing else",
		store.ErrSchemaVersion, path, found, schemaVersion)
}
