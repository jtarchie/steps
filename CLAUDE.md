...
`README.md` has the quick-start; `PRODUCT.md` is a product-framing doc, not engineering reference. `examples/*.yml` are runnable, self-contained example pipelines (several self-verifying via `steps test`) — `review-pipeline.yml` is the realistic composite (get → task → agent → put), the rest are one per feature area: `flow.yml` (conditionals/hooks/transitions), `attempts.yml` (retry/timeout), `agents.yml` (tool guards, sub-delegation, MCP grants, verdict routing — needs a live model), `infra.yml` (triggers, containers), `workspace.yml` (isolation). Agent examples take their model from `defaults.model`, overridable with `$STEPS_MODEL`.
