...
- **`internal/shell.HostRunner` does not inherit the full process environment.** `HostEnv()` (`internal/shell/shell.go`, exported specifically so `internal/mcp` can reuse it — see below) filters `os.Environ()` down to a fixed allowlist (`hostEnvAllowlist`: `PATH`, `HOME`, locale/terminal vars, `TMPDIR`/`USER`/`SHELL`, `SSH_AUTH_SOCK`, proxy vars) before every host-executed command — resource `check`/`in`/`out`, a task's `run:`, an agent's `run_shell`/custom tools, and a `command:`-configured `mcp_servers:` entry's stdio subprocess (`internal/mcp/stdio.go`). This is deliberate: an agent step's `run_shell`/custom tools (and a stdio MCP server, which a model then talks to) are commands a model directs, and the full environment previously included every configured agent's `api_key_env` secret and anything else the operator exported. If a host-executed command in a new feature needs some other ambient variable, add it to `hostEnvAllowlist` explicitly and say why — don't reach for `os.Environ()`/`cmd.Env = nil` as a shortcut, and don't assume a pipeline can read an arbitrary exported credential just because the `steps` process can see it. There is currently no per-pipeline pass-through mechanism (e.g. an `env:` block) for a variable outside the allowlist; that would be a real feature (its own config surface, merkle-hash implications) if ever needed — a stdio MCP server needing a credential is the concrete case this currently blocks.
...
internal/resource, internal/workspace, internal/merkle, internal/mcp                  -> config (+ shell/template/mcp for resource; merkle -> resource too; mcp -> shell for HostEnv, its stdio transport's env filtering)
...
There are five example pipelines, grouped by theme + runtime (consolidated from ten single-feature files):
...
- **examples/attempts.yml** — **Retry and timeout on all step types, self-verifying, modelless.** Every job is a `steps test` regression fixture demonstrating `attempts:` (retry count, up to N times per step) and `timeout:` (per-attempt wall-clock deadline) on get/task/put/agent steps. Shows: a task retrying multiple times and eventually succeeding (via shared state across attempts), a task exhausting retries, a timeout deadline, and a task combining both features. Log output shows `attempt N/M` markers and `retry.attempt_failed` warnings for each failed retry. Run `steps test examples/attempts.yml` or `steps run examples/attempts.yml --job <name>`. No network, credentials, or model.
...
8. **A behavior or guard added to one of several structurally similar code paths must be verified on all of them it claims to cover.** `steps` has multiple places where get/task/put/agent steps (or host/docker runners, or check/in/out commands) each get their own near-identical implementation of the same idea — retry loops, timeout handling, error classification. A fix landed on one of these paths (e.g. the `shell.CanceledError` guard that keeps a timed-out command from being misclassified as a task-level `Failure`) is easy to add to the one path you're looking at and forget to add to its siblings, and `go test`/`golangci-lint` won't catch the omission — nothing asserts the paths stay in sync. Before calling a change to one of these paths done, grep for its sibling call sites and check the same guard/behavior landed on each; prefer extracting a shared helper (see `internal/pipeline/pipeline.go`'s `retryWithTimeout`) over four copies that can drift. A code review of the attempts/timeout feature (2026-07-24) found exactly this pattern: the timeout-classification guard was on the task path but missing from put and get-in.
