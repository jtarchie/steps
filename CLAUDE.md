...
### Root Files
- **main.go** — CLI entry point; parses args into `run`/`watch`/`test` subcommands (`RunCmd`/`WatchCmd`/`TestCmd`) — `run` calls `config.LoadConfig()` → `pipeline.RunJob()`; `watch` → `trigger.Watch()` (see [docs/infra.md](docs/infra.md)); `test` runs every job (force) and verifies `assert:` directives (see [docs/control-flow.md](docs/control-flow.md)). A global `--log-level`/`STEPS_LOG_LEVEL` flag (default `info`) controls `initLogging`'s handler level — see "Trust Boundaries Around Pipeline-Defined and Model-Directed Execution" above.
...
- **[docs/control-flow.md](docs/control-flow.md)** — hooks (`on_success`/`on_failure`/`ensure`), conditional steps (`when:`), step transitions/verdict routing (`to:`/`max_visits:`/`verdicts:`), and self-verification (`assert:` + `steps test`).
