...
`README.md` has the quick-start; `PRODUCT.md` is a product-framing doc, not engineering reference. `examples/*.yml` are runnable, self-contained example pipelines (several self-verifying via `steps test`) — `review-pipeline.yml` is the realistic composite (get → task → agent → put), the rest are one per feature area: `flow.yml` (conditionals/hooks/transitions), `in-parallel.yml` (concurrent branches), `race.yml` (first success wins), `across.yml` (matrix cells), `attempts.yml` (retry/timeout), `agents.yml` (tool guards, sub-delegation, MCP grants, verdict routing — needs a live model), `infra.yml` (triggers, containers), `workspace.yml` (isolation). Agent examples take their model from `defaults.model`, overridable with `$STEPS_MODEL`. **`examples/invalid/*.yml` is the inverse**: pipelines that must FAIL `LoadConfig`, each with a `# expect: <substring>` line naming the error it has to produce (`examples_invalid_test.go`). That is where a load-time rejection gets a readable file; the four `examples/*.yml` globs are non-recursive precisely so these stay out of them.
