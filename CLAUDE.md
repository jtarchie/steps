...
- **Mutation testing guards the doc corpus, and it is the reason a doc example is a test rather than a decoration.** Two suites, both in the root package, both included in `go test ./...` (~20s of the root package's runtime; `task mutate` runs just them):
