...
**Two front ends, one model.** `internal/web` (echo + `html/template` + one CSS file, all `go:embed`ed) serves the browser UI, and `internal/events` is the stdlib-only leaf carrying run events between them. The rules that keep this from becoming a second product:
...
- **events must stay a leaf.** Every execution package publishes to it; the moment it imports config or store, they all inherit that edge and the graph stops being acyclic.
...
Config files: **`.golangci.yml`** (lint rules, and the `depguard` allow-lists that ARE the dependency graph — read it there rather than from a copy that can drift) — this *is* the pre-check-in validation pipeline, there's no separate CI file. **`go.mod`** pins the Go version and every dependency.

`README.md` has the quick-start; `PRODUCT.md` is a product-framing doc, not engineering reference. `examples/*.yml` are runnable, self-contained example pipelines, one per feature area (several self-verifying via `steps test`). `TestExamplesRun` runs every one of them and is **opt-out**: a file that needs something the test host hasn't got says so in a `# steps-test: skip -- <reason>` marker on its first line, and today six do — `agents.yml`, `cli-agent.yml`, `infra.yml`, `pr-review.yml`, `review-pipeline.yml`, `workspace.yml` (a live model, the `claude` CLI, a docker daemon or real API, the network, or a verdict-dependent outcome that could never be a deterministic fixture). `pr-review.yml`'s shape is instead pinned deterministically by `e2e_pr_review_test.go` against the fake provider. Opt-out is deliberate: an unmarked example that does not run is a failure, which is how a shipped example calling `make build` in a repo with no Makefile was caught. Agent examples take their model from `defaults.model`, overridable with `$STEPS_MODEL`. **`examples/invalid/*.yml` is the inverse**: pipelines that must FAIL `LoadConfig`, each with a `# expect: <substring>` line naming the error it has to produce (`examples_invalid_test.go`). That is where a load-time rejection gets a readable file; the four `examples/*.yml` globs are non-recursive precisely so these stay out of them.

`docs/*.md` is the feature-by-feature reference, indexed by [docs/README.md](docs/README.md) — not force-loaded into every task, so read the one relevant to what you're touching: [resources.md](docs/resources.md), [agents.md](docs/agents.md) (authoring) and [agents-internals.md](docs/agents-internals.md) (transport/compaction/loop detection), [control-flow.md](docs/control-flow.md), [infra.md](docs/infra.md), [workspace.md](docs/workspace.md), [templating.md](docs/templating.md), [mcp.md](docs/mcp.md), [web.md](docs/web.md), [conformance.md](docs/conformance.md).
