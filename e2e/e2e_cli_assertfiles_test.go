...
`, marker, cliResultEvent("Written.", 1), cliResultEvent("Here is the answer: it comes from widgets.json.", 1)))
	...
	if got := claude.invocations(t); got != 2 {
		t.Fatalf("the fake claude ran %d times, want 2 (the silent answer and the nudge)", got)
	}

	// Rejoined, not restarted — the session the first invocation opened is
	// the one the second continues.
	first, second := claude.argv(t, 1), claude.argv(t, 2)
	...
	if nudge := claude.prompt(t, 2); !strings.Contains(nudge, "answer/reply.md") {
...
func TestE2ECLIAgentStillFailsWithoutItsFile(t *testing.T) {
	...
	got := claude.invocations(t)
	if got < 2 {
		t.Errorf("the fake claude ran %d times, want the first answer plus at least one nudge", got)
	}

	if got > 6 {
		t.Errorf("the fake claude ran %d times, want no more than 6 — the nudge is bounded", got)
	}
...
`, counter, cliResultEvent("The answer is in this message.", 1)))
	...
	if got, want := claude.invocations(t), 2+5; got > want {
		t.Errorf("the fake claude ran %d times, want no more than %d — the two budgets pool rather than multiply", got, want)
	}
}
