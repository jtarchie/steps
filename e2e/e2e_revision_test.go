...
func TestRunsReportTheConfigTheyRan(t *testing.T) {
	...
	out := captureStdout(t, func() {
		runErr = cli.Run([]string{"runs", "-p", cli.PipelineName(pipeline), "--db", state})
	})
...
func TestResumeRecordsTheConfigurationThatFixedIt(t *testing.T) {
	...
	broken := configColumn(t, captureStdout(t, func() {
		err := cli.Run([]string{"runs", "-p", cli.PipelineName(path), "--db", state})
		if err != nil {
	...
	// The column prints a prefix of the sha rather than the whole of it —
	// how long a prefix is internal/cli's business (and pinned there), so
	// this asserts the cell names the fixed configuration and nothing about
	// its width.
	if !strings.HasPrefix(after.Revision.SHA, fixed[0]) {
		t.Errorf("the resumed run reports %q, want the configuration it resumed with (%q)", fixed[0], after.Revision.SHA)
	}
}
