...
func TestEndToEndPRReviewExample(t *testing.T) {
	...
	go func() {
		done <- cli.Run([]string{"test", path, "--var", "pr_repo=jtarchie/steps"})
	}()
...
func approveWhenPending(pipelinePath string, stop <-chan struct{}) error {
	...
	for time.Now().Before(deadline) {
		...
		select {
		case <-stop:
			return last //nolint:wrapcheck // the caller reports it as the approval that never landed
		case <-time.After(20 * time.Millisecond):
		}
	}

	return last //nolint:wrapcheck // as above
}
