...
func (c fakeCLI) argv(t *testing.T, n int) string {
	...
	if n > len(argvs) {
		t.Fatalf("the fake claude ran %d times, wanted invocation %d", len(argvs), n)
	}
...
func (c fakeCLI) prompt(t *testing.T, n int) string {
	...
	if n > len(prompts) {
		t.Fatalf("the fake claude ran %d times, wanted invocation %d", len(prompts), n)
	}
