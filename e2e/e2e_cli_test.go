...
func TestE2ECLIAgentInvocation(t *testing.T) {
	...
	_ = cli.Run([]string{path})

	argv := claude.argv(t, 1)
	...
	if prompt := claude.prompt(t, 1); !strings.Contains(prompt, "Review the diff.") {
		t.Errorf("the prompt did not reach the claude's stdin:\n%s", prompt)
	}
}
...
`+callBridgeScript("verdict", `{"choice":"approve"}`)+
	...
	if got := claude.invocations(t); got != 2 {
		t.Errorf("the claude ran %d times, want 2 (one failed attempt, one retry)", got)
	}
	...
	opening, retried := claude.argv(t, 1), claude.argv(t, 2)
	...
	if prompt := claude.prompt(t, 2); strings.Contains(prompt, "Review the diff.") {
...
func TestE2ECLIAgentDoesNotRetryTaskFailure(t *testing.T) {
	...
	err := cli.Run([]string{path})
	if err == nil {
		t.Fatal("a claude that reported the task as failed produced a successful run")
	}

	if got := claude.invocations(t); got != 1 {
		t.Errorf("the claude ran %d times, want 1 — a reported task failure must not be retried", got)
	}
	...
	if !strings.Contains(agentNode.Error, "Reached maximum number of turns") {
		t.Errorf("recorded error = %q, want the claude's own message rather than a bare subtype", agentNode.Error)
	}
}
...
`, filepath.Join(dir, "joined.log"))
	...
	if got := claude.invocations(t); got != 3 {
		t.Fatalf("the claude ran %d times, want one per cell (3)", got)
	}
	...
	for _, argv := range claude.records(t, "argv") {
