...
`, marker, cliUsageOnlyRun(50_000, 5_000), cliResultEvent("done", 2)))
	...
	if got := claude.invocations(t); got != 2 {
		t.Fatalf("the fake claude ran %d times, want 2 (a crash and its retry)", got)
	}

	first, ok := budgetArg(t, claude.argv(t, 1))
	if !ok {
	...
	second, ok := budgetArg(t, claude.argv(t, 2))
	if !ok {
...
func TestE2ECLIAgentBudgetExhaustionFailsTheStep(t *testing.T) {
	...
	claude := writeFakeClaude(t, "echo '"+cliUsageOnlyRun(100_000, 20_000)+"'\nexit 1")
	...
	if got := claude.invocations(t); got != 1 {
		t.Errorf("the fake claude ran %d times, want 1 — an exhausted budget stops the attempts", got)
	}
}
