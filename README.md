...
| `steps serve [--addr host:port] [--hook wf.ts ...] [--hook-input k=v] [--hook-token t] [--max-in-flight N]` | Web view of runs (default `127.0.0.1:8484`): list with status filters and per-machine token/cost totals, run detail (inputs, a per-execution table, a chronological timeline threading each state's prompts/replies/tools and the transition it took, artifacts, and journaled output/content), and a form to answer parked gates. `--hook` is repeatable: each machine that declares a `webhook:` block is triggerable at `POST /hooks/<path>`, which maps the JSON payload to run inputs and **durably queues** a run (`202`; `429` when the hook's queue is full). Per-hook `maxInFlight`/`maxQueued` bound each hook; `--max-in-flight` caps all hooks; `--hook-token path=secret` scopes a secret (bare value = fallback). See [docs/webhook.md](docs/webhook.md). |
