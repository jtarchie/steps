...
func TestEnsembleUnanimousFailsOnDisagreement(t *testing.T) {
	...
	err := run([]string{"run", ensemblePipeline(t, dir, members(t, "approve", "approve", "reject"), "unanimous", ""), "--job", "review"})
	if err == nil {
...
func TestEnsembleMemberErrorFailsByDefault(t *testing.T) {
	...
	err := run([]string{"run", ensemblePipeline(t, dir, members(t, "approve", "approve", ""), "majority", ""), "--job", "review"})
	if err == nil {
